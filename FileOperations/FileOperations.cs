    ...
    public static class FileOperations
    {
        ...
        private static bool CopyFilesInternal(List<string> fromList, List<string> toList, FILE_OP_TYPE opType)
        {
            ...
            SHFILEOPSTRUCT lpFileOp = new SHFILEOPSTRUCT()
            {
                ...
                pFrom = from,
                pTo = to,
                fFlags = FILE_OP_FLAGS.FOF_NOCONFIRMMKDIR | FILE_OP_FLAGS.FOF_MULTIDESTFILES | FILE_OP_FLAGS.FOF_NOCOPYSECURITYATTRIBS
            };
            ...
            if (SHFileOperation(ref lpFileOp) == 0)
            {
                ...
                if (success)
                {
                    foreach (string copiedFile in toList)
                    {
                        try { new FileInfo(copiedFile).Attributes = FileAttributes.Normal; }
                        catch { }
                    }
                }
            }
        ...
        public static void ExecuteFile(string path, string args = null, bool quoteArgs=true, string cwd = null)
        {
            System.Diagnostics.ProcessStartInfo si = new System.Diagnostics.ProcessStartInfo()
            {
                FileName = path,
                Arguments = (!string.IsNullOrWhiteSpace(args) && quoteArgs) ? $"\"{args}\"" : args,
